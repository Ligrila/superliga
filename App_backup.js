export default class App extends React.Component {
    ...
    _handleRedirect = async (event, initialUrl = false) => {
      ...
      if (
        ...
        urlToParse = urlToParse.replace(
          "exps://www.jugadasuperliga.com/",
          "jugadasuperliga://"
        );
        urlToParse = urlToParse.replace(
          "exps://jugadasuperliga.com/",
          "jugadasuperliga://"
        );
        urlToParse = urlToParse.replace(
          "https://www.jugadasuperliga.com/",
          "jugadasuperliga://"
        );
        urlToParse = urlToParse.replace(
          "https://jugadasuperliga.com/",
          "jugadasuperliga://"
        );
        urlToParse = urlToParse.replace(
          "http://www.jugadasuperliga.com/",
          "jugadasuperliga://"
        );
        urlToParse = urlToParse.replace(
          "http://jugadasuperliga.com/",
          "jugadasuperliga://"
        );
        urlToParse = urlToParse.replace(
          "jugadasuperliga.com/",
      ...
      if (path) {
        ...
        if (parts[0] == "championships" && parts[1]) {
          ...
          if (isLogin) {
            this.navigate("ChampionshipSubscribe", {
              championship: { id: championshipId },
            });
          } else {
            AsyncStorage.setItem(
              "afterLoginChampionshipSubscribe",
              championshipId
            );
    ...
    componentDidMount() {
      try {
        ...
        UsersActions.isLoggedIn.listen((b) => {
          //console.log({b});
          if (b) {
            this.initSocket();
          } else {
          ...
          registerPushNotifications()
            .then((data) => console.log("PushNotificationsRegister", data))
            .catch(function (e) {
              //rejected
            });
        });
